...
for ((i=0;i<$CYCLE;i++))
  do
	...
	qsub -q snoplsSL6 -m ea -o ${USERDIR} -e ${USERDIR}/${JOBDESC}_Run${i}_err.txt ${JOBDESC}_${i}_${JOBID}.sh
	echo "Submitted Final Job: "${i} " containing " ${EVPERCYCLE} " events containing e-mail notification."

    else
	chmod +x ${JOBDESC}_${i}_${JOBID}.sh  
	qsub -q snoplsSL6 -o ${USERDIR} -e ${USERDIR}/${JOBDESC}_Run${i}_err.txt ${JOBDESC}_${i}_${JOBID}.sh
